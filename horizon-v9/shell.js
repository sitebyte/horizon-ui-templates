/* ============================================================
   HORIZON ETRM v2 — Shared Shell Builder
...
function buildSidebar(activePage) {
  ...
  // Role selector (no brand area — toggle is now in the header)
...
function toggleSidebar() {
  ...
  // Update the toggle button icon to reflect new state
  var toggleBtn = document.getElementById('hz-sidebar-toggle');
  if (toggleBtn) {
    toggleBtn.innerHTML = svg(!exp ? 'chevron-left' : 'chevron-right', 16);
  }
}
...
function buildHeader(pageTitle) {
  ...
  html += '  <button class="hz-header-toggle" id="hz-sidebar-toggle" onclick="toggleSidebar()" title="Toggle sidebar (Cmd+B)">' + svg(sidebarExp ? 'chevron-left' : 'chevron-right', 16) + '</button>';
...
function buildStatusBar() {
  ...
  html += '<span style="margin-left:auto;">Horizon ETRM v9.0</span>';
  html += '</div>';
  return html;
}
