/* ============================================================
   HORIZON ETRM v31.0 — AI-Aware Shared Shell Builder
...
/* ========== EOS MENU (v21 — 6 groups, every EOS item, zero additions) ========== */
var MENU = [
  ...
  { group: 'OPERATIONS', defaultOpen: false, items: [
    ...
    { label: 'Cargo Audit Summary', icon: 'clock', href: 'audit-log.html', key: 'audit-log' },
    { label: 'Shipments', icon: 'navigation', href: 'nominations.html', key: 'nominations' },
  ]},
  { group: 'FINANCE', defaultOpen: false, items: [
    { label: 'Valuations', icon: 'trending-up', href: 'valuations.html', key: 'valuations' },
    { label: 'Statements', icon: 'file-text', href: 'invoices.html', key: 'invoices' },
    { label: 'Portfolio', icon: 'bar-chart-2', href: 'positions.html', key: 'positions' },
    { label: 'Prices', icon: 'trending-up', href: 'curves.html', key: 'curves' },
  ]},
  { group: 'ADMINISTRATION', defaultOpen: false, items: [
    {
      label: 'Static Data', icon: 'clipboard', key: 'static-data', children: [
        {
          label: 'General', key: 'static-general', children: [
        ...
        {
          label: 'Companies', key: 'static-companies', children: [
            { label: 'Manage Companies', href: 'settings.html#companies', key: 'ref-companies' },
        ...
        {
          label: 'Tax', key: 'static-tax', children: [
            { label: 'Manage Tax Rates', href: 'settings.html#tax-rates', key: 'ref-tax-rates' },
          ]
        },
        {
          label: 'Products', key: 'static-products', children: [
        ...
        {
          label: 'Prices', key: 'static-prices', children: [
        ...
        {
          label: 'Reports', key: 'static-reports', children: [
            { label: 'Manage Reports', href: 'settings.html#manage-reports', key: 'ref-manage-reports' },
          ]
        },
        {
          label: 'Security', key: 'static-security', children: [
    ...
    { label: 'My Profile', icon: 'user', href: 'user-profile.html', key: 'user-profile' },
  ]},
  { group: 'SYSTEM', defaultOpen: false, items: [
  ...
  { group: 'SUPPORT', defaultOpen: false, items: [
    ...
    { label: 'Environments', icon: 'settings', href: 'support-environments.html', key: 'support-environments' },
    { label: 'Audit Search', icon: 'search', href: 'support-audit.html', key: 'support-audit' },
    { label: 'Data Quality', icon: 'shield', href: 'support-data-quality.html', key: 'data-quality' },
...
function toggleMenuGroup(key) {
  ...
  document.querySelectorAll('.hz-menu-group').forEach(function(g) {
    if (g !== group) {
      var gItems = g.querySelector('.hz-menu-group-items');
      var gArrow = g.querySelector('.hz-menu-group-arrow');
      var gBtn = g.querySelector('.hz-menu-group-btn');
      if (gItems) gItems.classList.remove('open');
      if (gArrow) gArrow.classList.remove('open');
      if (gBtn) gBtn.setAttribute('aria-expanded', 'false');
      state[g.dataset.group] = false;
    }
  });
...
function buildHeader(pageTitle) {
  ...
  html += '  <div class="hz-avatar-wrap">';
  html += '    <div class="hz-avatar" title="J. Cobley" tabindex="0" role="button" aria-label="User menu" onclick="toggleUserMenu(event)">JC</div>';
  html += '    <div id="hz-user-menu" class="hz-user-menu" data-component="user-menu" style="display:none">';
  html += '      <div class="hz-user-menu-header">';
  html += '        <div class="hz-user-menu-avatar">JC</div>';
  html += '        <div class="hz-user-menu-identity">';
  html += '          <div class="hz-user-menu-name">Jonathan Cobley</div>';
  html += '          <div class="hz-user-menu-email">j.cobley@glng.com</div>';
  html += '        </div>';
  html += '      </div>';
  html += '      <div class="hz-user-menu-meta">';
  html += '        <div class="hz-user-menu-meta-row">' + svg('briefcase', 12) + '<span>Senior LNG Trader</span></div>';
  html += '        <div class="hz-user-menu-meta-row">' + svg('users', 12) + '<span>GLNG Trading Desk</span></div>';
  html += '        <div class="hz-user-menu-meta-row">' + svg('shield', 12) + '<span>Trader, Admin</span></div>';
  html += '      </div>';
  html += '      <div class="hz-user-menu-divider"></div>';
  html += '      <a class="hz-user-menu-item" href="user-profile.html">' + svg('user', 14) + '<span>View Profile</span></a>';
  html += '      <a class="hz-user-menu-item" href="settings.html">' + svg('settings', 14) + '<span>Settings</span></a>';
  html += '      <div class="hz-user-menu-divider"></div>';
  html += '      <a class="hz-user-menu-item hz-user-menu-item-danger" href="signin.html">' + svg('log-out', 14) + '<span>Sign Out</span></a>';
  html += '    </div>';
  html += '  </div>';
  html += '</div>';
  html += '</header>';
...
function toggleUserMenu(e) {
  e.stopPropagation();
  var menu = document.getElementById('hz-user-menu');
...
var CMD_ACTIONS = [
  ...
  { label: 'Go to My Profile', keys: 'G U', group: 'Navigate', action: function() { location.href = 'user-profile.html'; } },
...
function buildStatusBar() {
  ...
  html += '<span style="margin-left:auto;">Horizon ETRM v31.0</span>';
  html += '</div>';
  return html;
}
...
function toggleAppFullscreen() {
  ...
  var exitBtn = document.getElementById('hz-fullscreen-exit');

  if (document.body.classList.contains('hz-app-fullscreen')) {
    document.body.classList.remove('hz-app-fullscreen');
    if (btn) { btn.innerHTML = svg('maximize', 14); btn.title = 'Toggle fullscreen (F11)'; }
    if (exitBtn) exitBtn.remove();
  } else {
    document.body.classList.add('hz-app-fullscreen');
    if (btn) { btn.innerHTML = svg('minimize', 14); btn.title = 'Exit fullscreen (F11)'; }
    // Add floating exit button
    if (!document.getElementById('hz-fullscreen-exit')) {
      var exit = document.createElement('button');
      exit.id = 'hz-fullscreen-exit';
      exit.className = 'hz-fullscreen-exit-btn';
      exit.innerHTML = svg('minimize', 14) + ' <span>Exit Fullscreen</span>';
      exit.title = 'Exit fullscreen (Esc or F11)';
      exit.onclick = toggleAppFullscreen;
...
function initShell(activePage, pageTitle) {
  ...
  document.addEventListener('keydown', function(e) {
    ...
    // Escape
    if (e.key === 'Escape') { closeCmdK(); closeUserMenu(); exitFullscreenIfActive(); }
    // F11
  ...
  document.addEventListener('click', function(e) {
    ...
    if (menu && menu.style.display !== 'none') {
      if (!e.target.closest('.hz-avatar-wrap')) {
        closeUserMenu();
      }
    }
  });
