/* ============================================================
   HORIZON ETRM v8 — Shared Shell Builder
   Single import: initShell(activeKey, pageTitle)
   Builds: top nav bar, header, cmd-k, toast, theme toggle, status bar
   No sidebar — navigation is integrated into a horizontal nav bar
   ============================================================ */
...
const MENU = [
  ...
  },
  {
    group: 'PHYSICAL', defaultOpen: false, items: [
      ...
      { label: 'Vessel Charters', icon: 'anchor', href: 'vessel-charters.html', key: 'vessel-charters' },
  ...
  },
  {
    group: 'SETTLEMENT', defaultOpen: false, items: [
      ...
      { label: 'Charge Management', icon: 'dollar', href: 'charges.html', key: 'charges' },
  ...
  },
  {
    group: 'ADMIN', defaultOpen: false, items: [
      ...
      {
        label: 'Reference Data', icon: 'clipboard', key: 'reference-data', children: [
          ...
          { label: 'Products', href: 'settings.html', key: 'ref-products' },
          { label: 'Attributes', href: 'settings.html', key: 'ref-attributes' },
        ]
      },
...
function buildNavBar(activePage) {
  var html = '<nav class="hz-nav-bar">';
  MENU.forEach(function(group) {
    var groupHasActive = group.items.some(function(item) {
      return isItemActive(item, activePage) || hasActiveDescendant(item, activePage);
    });
    html += '<div class="hz-nav-group' + (groupHasActive ? ' active' : '') + '" onclick="toggleNavGroup(this)">';
    html += group.group;
    html += '<div class="hz-nav-dropdown">';
    group.items.forEach(function(item) {
      var active = isItemActive(item, activePage);
      html += '<a class="hz-nav-link' + (active ? ' active' : '') + '" href="' + (item.href || '#') + '">';
      html += '<span>' + item.label + '</span>';
      if (item.badge) {
        html += '<span class="nav-badge ' + item.badge.color + '">' + item.badge.count + '</span>';
      }
      html += '</a>';
      // Render children as indented links
      if (item.children) {
        item.children.forEach(function(child) {
          var childActive = isItemActive(child, activePage);
          html += '<a class="hz-nav-link' + (childActive ? ' active' : '') + '" href="' + (child.href || '#') + '" style="padding-left:var(--sp-6);font-size:0.625rem;">';
          html += child.label;
          html += '</a>';
        });
      }
...
function toggleNavGroup(el) {
  ...
  document.querySelectorAll('.hz-nav-dropdown.open').forEach(function(d) {
    if (d.parentElement !== el) d.classList.remove('open');
  });
  var dropdown = el.querySelector('.hz-nav-dropdown');
  dropdown.classList.toggle('open');
}

// Close nav dropdowns when clicking outside
document.addEventListener('click', function(e) {
  if (!e.target.closest('.hz-nav-group')) {
    document.querySelectorAll('.hz-nav-dropdown.open').forEach(function(d) {
...
function buildHeader(pageTitle) {
  ...
  html += '  <button class="hz-cmd-k-btn" onclick="openCmdK()" title="Cmd+K">' + svg('search', 14) + ' <span class="hz-kbd">&#8984;K</span></button>';
  html += '  <button class="hz-notif-btn" title="Notifications">' + svg('alert-circle', 14) + '<span class="hz-notif-badge">3</span></button>';
  html += '  <span class="hz-env-badge prod">PROD</span>';
  html += '  <button class="hz-theme-btn" onclick="toggleTheme()" title="Toggle theme"><span class="hz-theme-icon-header">' + svg('moon', 14) + '</span></button>';
  html += '  <div class="hz-avatar" title="J. Cobley">JC</div>';
  html += '</div>';
  html += '</header>';
  return html;
}
...
function buildContextStrip() {
  var html = '<div class="hz-context-strip">';
...
function buildStatusBar() {
  var html = '<div class="hz-status-bar">';
  html += '<span class="flex items-center gap-1"><span class="hz-dot green pulse" style="width:0.375rem;height:0.375rem;"></span> Connected</span>';
  html += '<span>Last update: ' + new Date().toLocaleTimeString('en-GB', {hour:'2-digit',minute:'2-digit',second:'2-digit'}) + '</span>';
  html += '<span style="margin-left:auto;">Horizon ETRM v8.0</span>';
  html += '</div>';
  return html;
}
...
function initShell(activePage, pageTitle) {
  ...
  // Build the shell — header + context strip + nav bar + content + status bar
...
function updateThemeIcons() {
  ...
  document.querySelectorAll('.hz-theme-icon-header').forEach(function(el) {
