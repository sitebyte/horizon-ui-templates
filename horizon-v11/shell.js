/* ============================================================
   HORIZON ETRM v11.0 — AI-Aware Shared Shell Builder
...
/* ========== FULL ETRM MENU (v11 — AI-aware, EOS-complete) ========== */
var MENU = [
  {
...
function buildSidebar(activePage) {
  ...
  var expanded = localStorage.getItem('hz-sidebar-expanded') !== 'false';
  var role = localStorage.getItem('hz-role') || 'trader';

  var html = '<div id="hz-sidebar" class="hz-sidebar ' + (expanded ? 'expanded' : '') + '" data-expanded="' + expanded + '" data-component="sidebar" role="navigation" aria-label="Main navigation">';

  // Role selector
  html += '<div class="hz-role-selector">';
  html += '  <select class="hz-role-select" id="hz-role-select" onchange="setRole(this.value)" tabindex="0" aria-label="Select role">';
  ...
  html += '  <button class="hz-menu-item" onclick="toggleTheme()" data-tooltip="Toggle Theme" tabindex="0">';
...
function buildHeader(pageTitle) {
  ...
  var sidebarExp = localStorage.getItem('hz-sidebar-expanded') !== 'false';
  html += '  <button class="hz-header-toggle" id="hz-sidebar-toggle" onclick="toggleSidebar()" title="Toggle sidebar (Cmd+B)" tabindex="0" aria-label="' + (sidebarExp ? 'Collapse sidebar' : 'Expand sidebar') + '">' + svg(sidebarExp ? 'chevrons-left' : 'chevrons-right', 16) + '</button>';
...
function buildStatusBar() {
  ...
  html += '<span style="margin-left:auto;">Horizon ETRM v11.0</span>';
  html += '</div>';
  return html;
}
